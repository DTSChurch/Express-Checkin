...
{
    [DisplayName("QR Express Person Select (Family Check-in)")]
...
", "", 6, "OptionFormat" )]
    ...
    public partial class QRExpressMultiPersonSelect : CheckInBlock
    {
            ...
            if (KioskCurrentlyActive)
            {
                ...
                if (family != null)
                {
                    foreach (var person in family.People)
                    {
                        ...
                        var personIds = attendanceService.Queryable().AsNoTracking()
                            .Where(a =>
                               a.ForeignKey == CurrentCheckInState.CheckIn.SearchValue)
                            .Select(a => a.PersonAlias.PersonId);

                        if (personIds.Contains(person.Person.Id))
                        {
