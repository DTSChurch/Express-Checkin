...
{
    [DisplayName( "QR Express Welcome" )]
    [Category("com_DTS > Check-in")]
    [Description( "Welcome screen for check-in with QR Code scanning capabilities." )]
    ...
    [CodeEditorField(
        "QR Code Title",
        Key = AttributeKey.QRCodeScanTitle,
        Description = "The text to display to indicate to the user to scan the QR Code for contactless checkin",
        IsRequired = false,
        DefaultValue = "Scan here",
        Category = "Text",
        Order = 15)]
    ...
    public partial class QRExpressWelcome : CheckInBlock
    {
        ...
        protected static class AttributeKey
        {
            ...
            public const string QRCodeScanTitle = "QRCodeScanTitle";
        }
...
", CurrentTheme, CurrentKioskId, CurrentCheckinTypeId, CurrentGroupTypeIds.AsDelimited(","));
            ...
            else
            {
                if (Request.Form["__EVENTARGUMENT"] != null)
                {
                    ...
                    //Used for QR Checkin
                    else if (Request.Form["__EVENTARGUMENT"] == "Attendance_Foreign_Key_Search")
                    {
                        //litLoadingSpinner.Text = "<i class='fas fa-sync-alt fa-3x fa-spin'></i>";
                        lScanTextHtml.Visible = false;
                        RefreshView();
                        //plugin defined defined value guid
                        var dv = DefinedValueCache.Get(ContactlessCheckinGuids.CHECKIN_SEARCH_TYPE_QR);
                        DoAttendanceForeignKeySearch(dv, hfSearchEntry.Value);
                    }
        ...
        private void DoAttendanceForeignKeySearch(DefinedValueCache searchType, string searchValue)
        {
            ...
            errors = ProcessSelection(maWarning);
            if (errors.Any())
            {
                ClearSelection();
        ...
        private void RefreshView()
        {
            ...
            //added qr code scan text
            lScanTextHtml.Text = GetAttributeValue(AttributeKey.QRCodeScanTitle).IfEmpty("Scan here");
        }
