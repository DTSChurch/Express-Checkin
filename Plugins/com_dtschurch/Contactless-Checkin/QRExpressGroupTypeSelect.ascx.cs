...
{
    [DisplayName( "QR Express Group Type Select" )]
    ...
    public partial class QRExpressGroupTypeSelect : CheckInBlockMultiPerson
    {
        /// <summary>
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            else
            {
                if ( !Page.IsPostBack )
                {
                    ...
                    var groupTypeFromQR = attendanceService.Queryable()
                                       .Where(a =>
                                          a.ForeignKey == CurrentCheckInState.CheckIn.SearchValue)
                                       .Select(a => a.Occurrence.Group.GroupTypeId).FirstOrDefault();
                    availGroupTypes.RemoveAll(gt => gt.GroupType.Id != groupTypeFromQR);
